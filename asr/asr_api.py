"""HTTPS-facing FastAPI adapter for the member-2 ASR integration.

TLS termination should be configured at the reverse proxy/cloud load balancer.
Run this application behind that proxy; do not put certificates in this file.
"""
...
try:
    from .asr_service import transcribe_event
except ImportError:  # supports ``uvicorn ai.asr_api:app``
    from asr_service import transcribe_event


load_dotenv(Path(__file__).with_name(".env"))
MAX_AUDIO_BYTES = int(os.getenv("ASR_MAX_AUDIO_BYTES", str(500 * 1024 * 1024)))
ALLOWED_SUFFIXES = {".mp3", ".wav", ".pcm", ".opus", ".flac", ".ogg", ".speex"}
...
app = FastAPI(title="Gongzai Internal ASR", version="1.0.0")
...
@app.post("/internal/ai/asr", response_model=ASRResponse)
async def asr(
    ...
    del recorded_at  # retained for the contract; provider does not need it
