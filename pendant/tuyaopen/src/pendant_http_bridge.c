...
static void poll_once(void)
{
    ...
    if (!sg_pending_moment) {
        ...
        (void)strncpy(sg_moment.event_id, event_id, sizeof(sg_moment.event_id) - 1U);
        sg_pending_moment = true;
        PR_NOTICE("FastAPI event received: %s, %d BPM", event_id, bpm);
    }
    tal_free(json);
}
...
static void http_worker(void *arg)
{
    (void)arg;
    for (;;) {
        if (network_is_up() && !sg_pending_moment) {
            poll_once();
        }
...
static void post_event_state(const char *event_id, const char *suffix, const char *json)
{
    char path[192];
    if (event_id == NULL || event_id[0] == '\0' || !network_is_up()) return;
    (void)snprintf(path, sizeof(path), "/api/pendant/events/%s/%s", event_id, suffix);
    (void)http_request_json("POST", path, json, NULL);
}

void pendant_http_bridge_ack(const char *event_id, const char *status)
{
    char body[192];
    (void)snprintf(body, sizeof(body), "{\"device_id\":\"%s\",\"status\":\"%s\"}",
                   GONGZAI_PENDANT_DEVICE_ID, status != NULL ? status : "played");
    post_event_state(event_id, "ack", body);
}
